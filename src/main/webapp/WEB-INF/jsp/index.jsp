...
<!--ToDO: DOCTYPE anpassen, sodass er auf IE und FF läuft-->
		...
		<script type="text/javascript">
			...
			<c:forEach var="o" items="${observations}">
					addMarker(
						...
						"<b><s:escapeBody htmlEscape="true" javaScriptEscape="true">${o.title}</s:escapeBody></b><br/><br/><s:escapeBody htmlEscape="true" javaScriptEscape="true">${o.description}</s:escapeBody><br/><br/>Koordinaten:<br/><small>Lon: ${o.coordinate.longitude} | Lat: ${o.coordinate.latitude}</small>",
						"${o.categorie.iconFileName}");
			</c:forEach>
				});
		</script>
	</head>
		...
		<table id="maintable" class="maintable" style="height: 100%; width: 100%;">
			<tr>
				<td colspan="2" class="header">
					<a href="<s:url value="/" htmlEscape="true" />">
						<img src="<s:url value="/static/img/logo.png" htmlEscape="true"/>" align="left" width="160" alt="Logo" />
					</a>
					<c:choose>
						<c:when test="${sessionScope.LOGIN != null}">
							<a href="<s:url value="/acc" />">
								<img src="<s:url value="/static/img/user_mng.png" htmlEscape="true"/>" alt="" height="18"/>
								<b>${sessionScope.LOGIN.mail}</b>
							</a> |
							<a href="<s:url value="/logout"/>">
								<fmt:message key="logout.button.title"/>
							</a>
						</c:when>
						<c:otherwise>
							<a href="<s:url value="/login"/>">
								<fmt:message key="login.button.title"/>
							</a>
						</c:otherwise>
					</c:choose>
					...
					<a href="<s:url value="/help" />">
						<img src="<s:url value="/static/img/help.png" htmlEscape="true"/>" alt="" height="18"/>
					</a>
					<hr style="border:solid #DDDDEE 1px"/>
					<br/>
					<br/>
				</td>
			</tr>
			...
			<tr class="content">
				<td class="sidebar">
					...
					<h4>
						<s:escapeBody htmlEscape="true">
							<fmt:message key="greeting.title"/>
						</s:escapeBody>
					</h4>
					<!-- Beginning of observationlist-->
					<div id="results" class="observationlist">
						<table id="observationTable" class="observationlist">
							<c:forEach var="o" end="8" items="${observations}">
								<tr class="entry">
									<td class="observationlist_title">
										...
										<a href="#" onclick="goTo(${o.coordinate.longitude}, ${o.coordinate.latitude}, 14);">
											<img src="<s:url value="/static/img/show.png" htmlEscape="true" />" alt="<s:escapeBody htmlEscape="true"><fmt:message key="observation.viewInMap.title"/></s:escapeBody>" align="right" height="20px"/>
										</a>
										<c:if test="${sessionScope.LOGIN != null}">
											<a href="<s:url value="/r/o/${o.id}/new"/>">
												<img src="<s:url value="/static/img/report.png" htmlEscape="true" />" alt="<s:escapeBody htmlEscape="true"><fmt:message key="observation.report.title"/></s:escapeBody>" align="right" height="20px"/>
											</a>
											<c:if test="${sessionScope.LOGIN.userGroup == 'ADMIN' || o.user.id == sessionScope.LOGIN.id}">
												<a href="<s:url value="/o/edit/${o.id}"/>">
													<img src="<s:url value="/static/img/edit.png" htmlEscape="true" />" alt="<s:escapeBody htmlEscape="true"><fmt:message key="observation.edit.title"/></s:escapeBody>" align="right" height="20px"/>
												</a>
											</c:if>
										</c:if>
