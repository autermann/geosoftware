...
<div>
	<h2><fmt:message key="observation.details.beobachtung.title"/> <c:out value="${observation.id}"/></h2>
	<table border="1">
		<tr>
			<td><fmt:message key="observation.id.title"/>:</td>
			<td>${observation.id}</td>
		</tr>
		<tr>
			<td><fmt:message key="observation.title.title"/>:</td>
			<td><s:escapeBody htmlEscape="true">${observation.title}</s:escapeBody></td>
		</tr>
		<tr>
			<td><fmt:message key="observation.description.title"/>:</td>
			<td><s:escapeBody htmlEscape="true">${observation.description}</s:escapeBody></td>
		<tr/>
		<tr>
			<td><fmt:message key="observation.user.title"/>:</td>
			<td>${observation.user}</td>
		<tr/>
		<tr>
			<td><fmt:message key="observation.creationDate.title"/>:</td>
			<td><fmt:formatDate pattern="dd. MM. yyyy - HH:mm" value="${observation.creationDate}"/></td>
		<tr/>
		<tr>
			<td><fmt:message key="observation.observationCategorie.title"/>:</td>
			<td><s:escapeBody htmlEscape="true">${observation.categorie}</s:escapeBody></td>
		<tr/>
		<tr>
			<td><fmt:message key="observation.coordinate.title"/>:</td>
