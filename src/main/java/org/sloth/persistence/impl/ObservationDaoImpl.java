...
public class ObservationDaoImpl extends EntityManagerDao<Observation>
	...
	public Observation getById(Long id) {
		...
		else logger.info("Can't find Observation with Id {}", id);
		return o;
	}

	@Override
	public void save(Observation o) {
		if (isAttached(o)) throw new EntityAlreadyKnownException();
	...
	public void update(Observation o) {
		if (!isAttached(o)) throw new EntityNotKnownException();
	...
	public Collection<Observation> getByCategorie(Categorie c) throws
		...
		if (c.isNew()) throw new IllegalArgumentException();
	...
	public Collection<Observation> getByUser(User u) throws NullPointerException,
		...
		if (u.isNew()) throw new EntityNotKnownException();
		CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
		CriteriaQuery<Observation> cq = cb.createQuery(Observation.class);
		Root<Observation> o = cq.from(Observation.class);
		cq.select(o);
	...
	public Collection<Observation> getByKeyWord(String key) throws NullPointerException {
		...
		Collection<Observation> result = getEntityManager().createQuery(q.select(o).distinct(true).where(
			b.or(
